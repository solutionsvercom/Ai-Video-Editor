    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      ...
      <div className="aspect-video relative overflow-hidden">
        ...
        {project.status === 'completed' && (
          <Link to={createPageUrl(`Editor?id=${project.id}`)} className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="bg-[#1a1a24] border-white/10 text-white">
              <DropdownMenuItem asChild className="hover:bg-white/5 cursor-pointer">
                <Link to={createPageUrl(`Editor?id=${project.id}`)}><Edit className="w-4 h-4 mr-2" />Edit</Link>
              </DropdownMenuItem>
